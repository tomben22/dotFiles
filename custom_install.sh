...
gem install deliver snapshot frameit sigh pem cert scan gym match pilot
...
hdiutil attach xca.dmg
cp -r /Volumes/`ls /Volumes/ | grep xca`/xca.app /Applications
hdiutil detach /Volumes/`ls /Volumes/ | grep xca`
